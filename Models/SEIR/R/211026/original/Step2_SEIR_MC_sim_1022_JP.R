...
cum_vac<-df_jp1[c(1,6,7)]
cum_vac<-subset(cum_vac, cum_vac$date >= "2021-07-01")
cum_vac$new_vac <- c(200000,diff(cum_vac$people_vaccinated))
cum_vac[cum_vac<0] <- 0
npop <- 126000000
vac<-cum_vac$new_vac/npop #country total pop

vr=1 #by vaccination rate: 0.8,1
vac1 <- function(t) (t >= 1 & t < 96) * mean(vac)*0.9+
  (t >= 96 & t < 180) * mean(vac)*vr*0.1+
  (t >= 180 & t < 360) * mean(vac)*0.1*vr+
...
test<-df_jp1[c(1,5)]
test[is.na(test)] <- 80000
test<-subset(test, test$date >= "2021-07-01")
tau2<-test$new_tests/250000*2.5 ## assumed total asymptomatic population
...
npi=1 #0.85
npi_d=150 ##NPI by April 2022 by Feb 2022 by Jun 2022

contactpattern <- function(t, aver_Rt) (t >= 1 & t < 15) * aver_Rt*1/14*(1/contact) + 
  (t >= 15 & t < 90) * aver_Rt*1/14*(1/contact)*4.5+   ##Rt1.1
  (t >= 90 & t < npi_d) * aver_Rt*1/14*(1/contact)*rt*npi*3+ ##Rt1.3
  (t >= npi_d & t  < lastdate) * aver_Rt*1/14*(1/contact)*rt*6 ##Transimissibility increaes  3.5/4/4.5 --> Rt: 2.8/3.2/3.7
...
V=c(724, 1257656, 1360168, 7290718)*2 #by end of June cumulative vaccine uptake ( 724 1257656 1360168 7290718)
E=c(1000, 2000, 2000, 1000)*2
A=c(1000, 2000, 2000, 1000)*1
TP=c(100, 1000, 1000, 1000) ##July fit c(100, 400, 300)
S=c(300, 800, 800, 800)
R=c(10000,2000, 80000, 150000)*2
D=c(0, 500, 1000, 1000)*2
FP=c(0, 0, 0, 0)
...
result2$TP1.incidence <- c(100*2.5,diff(result2$TP.I1))
result2$TP2.incidence <- c(100*2.5,diff(result2$TP.I2))
result2$TP3.incidence <- c(500*2.5,diff(result2$TP.I3))
result2$TP4.incidence <- c(300*2.5,diff(result2$TP.I4))
result2$TP.incidence <- c(1000*2.5,diff(result2$TP.I))
