...
vac1 <- function(t) (t >= 1 & t < 96) * mean(vac)*0.2+
  (t >= 96 & t < 180) * mean(vac)*vr*0.2+
  (t >= 180 & t < 360) * mean(vac)*vr+
  (t >= 360 & t < 540) * mean(vac)
# 
...
tau2<-test$new_tests/250000*0.5 ## assumed total asymptomatic population
...
contactpattern <- function(t, aver_Rt) (t >= 1 & t < 96) * aver_Rt*1/14*(1/contact)*0.2 + 
  (t >= 96 & t  < npi_d) * aver_Rt*1/14*(1/contact)*rt*npi*0.8+ ##Rt1.3
  (t >= npi_d & t  < lastdate) * aver_Rt*1/14*(1/contact)*rt ##Transimissibility increaes  3.5/4/4.5 --> Rt: 2.8/3.2/3.7
...
E=c(1000, 2000, 2000, 1000)*0.2
A=c(1000, 2000, 2000, 1000)*0.2
TP=c(100, 1000, 1000, 1000)*0.05 ##July fit c(100, 400, 300)
S=c(300, 800, 800, 800)
R=c(10000,2000, 80000, 150000)*1
D=c(0, 500, 1000, 1000)*1
FP=c(0, 0, 0, 0)
...
result2$TP1.incidence <- c(100*0.05,diff(result2$TP.I1))
result2$TP2.incidence <- c(100*0.05,diff(result2$TP.I2))
result2$TP3.incidence <- c(500*0.05,diff(result2$TP.I3))
result2$TP4.incidence <- c(300*0.05,diff(result2$TP.I4))
