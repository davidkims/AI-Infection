...
vr=1.5 #by test rate: 1,0.95,1.2
vac1 <- function(t, age.vac1) (t >= 1 & t < 96) * age.vac1 +
   (t >= 96 & t < 180) * age.vac1 *vr +
   (t >= 180 & t < 360) * age.vac1*0.25 *vr +
   (t >= 360 & t < lastdate) * age.vac1*0.1 *vr 

vac1_lhs <- function(t, age.vac1, vac_k) (t >= 1 & t < 96) * age.vac1*vac_k+
  (t >= 96 & t < 180) * age.vac1 *vac_k +
  (t >= 180 & t < 360) * age.vac1*0.25 *vac_k +
  (t >= 360 & t < lastdate) * age.vac1*0.1 *vac_k 
...
vac1_SA <- function(t, age.vac1, vaccinating) (t >= 1 & t < 96) * age.vac1 +
  ...
  (t >= 360 & t < lastdate) * age.vac1*0.1 *vaccinating 
...
contactpattern <- function(t, aver_Rt) (t >= 1 & t < 96) * aver_Rt*1/14*(1/contact) + ##Rt1.1
  (t >= 96 & t  < npi_d) * aver_Rt*1/14*(1/contact)*rt*npi+ ##Rt1.3
  (t >= npi_d & t  < lastdate) * aver_Rt*1/14*(1/contact)*rt*1 ##Rt2 with 1.2: Rt1.8 with 1.1: Rt1.65 with 1
...
pop.SI <- c(U = N-E-A-V-TP-S-R-D, V=V, E=E, A=A ,TP=TP, FP=FP,  S=S, R=R, D=D)
...
tau_k<-B[,1] <- qunif(A[,1], min = 0.98, max = 1.02)    
vac_k<-B[,2] <- qunif(A[,2], min = 0.9, max = 1.1) 
beta_k<-B[,3] <- qunif(A[,3], min = 0.98, max = 1.02) 

#result.lhs <-mapply(ts.sir.lhs,  tau_k=tau_k, beta_k=beta_k, SIMPLIFY=FALSE) 
result.lhs <-mapply(ts.sir.lhs, vac_k=vac_k, tau_k=tau_k, beta_k=beta_k, SIMPLIFY=FALSE) 
#result.lhs[[1]]
result.lhs.df <- as.data.frame(do.call("rbind", result.lhs))

result.lhs.low<-result.lhs.df%>%summarize_at(1:lastdate, ~quantile(., probs=0.2)) %>%  gather("column", "low")
result.lhs.high<-result.lhs.df%>%summarize_at(1:lastdate, ~quantile(., probs=0.8)) %>% gather("column", "high")
result.lhs.ci<-cbind(result.lhs.low,result.lhs.high)
result.lhs.ci <- result.lhs.ci[c(2,4)]
# result.lhs.ci$time<-data.frame(time = 1: length(t) )
# colnames(result.lhs.ci)[4]<-"time"
