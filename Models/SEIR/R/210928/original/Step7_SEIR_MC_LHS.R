...
tau2_1 <- function(t, tau_k) (t >= 1 & t < 80) *  tau_k*0.35 +
  (t >= 80 & t < lastdate) *  tau_k*0.35*1

vac1 <- function(t, vac_k) (t >= 1 & t < 87) * vac_k*0.7+
  (t >= 87 & t < lastdate) * vac_k*0.7*1

contactpattern <- function(t, beta_k) (t >= 1 & t < 80) * beta_k*0.35 +
  (t >= 80 & t < 87) * beta_k*0.45*1+
  (t >= 87 & t < 147) * beta_k*0.42*1+ 
  (t >=147 & t < lastdate) * beta_k*0.42*1
...
tau_k<-B[,1] <- qunif(A[,1], min = mean(tau2)*0.95, max = mean(tau2)*1.05)    
vac_k<-B[,2] <- qunif(A[,2], min = mean(vac)*0.95, max = mean(vac)*1.05) 
beta_k<-B[,3] <- qunif(A[,3], min = mean(beta)*0.98, max = mean(beta)*1.02) 
