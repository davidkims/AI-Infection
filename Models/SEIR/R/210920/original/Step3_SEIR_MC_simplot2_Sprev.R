...
inc_reinf <- (ggplot(q1) 
              + aes(x = dates) 
              + geom_ribbon(data = q1, aes(x = dates, ymin = TP-100, ymax = TP+100), alpha = .2)
              + geom_line(aes(y = TP), color = '4')
              + geom_point(aes(y = TP-50), size = .2, color = '4', alpha = .5)
              + geom_line(data = q1, aes(y = TP-50), color = 'darkgrey')
              + ylab('Incidence of reinfections')
              + xlab('Specimen receipt date')
              + geom_vline(aes(xintercept =  as.Date('2021-08-15')), linetype = 3, color = 'red')
              + scale_x_date(NULL, date_breaks = "months", date_labels = "%b", minor_breaks = NULL)
              #+ xlim(min(time),max(time))
              + ggtitle('Full course of epidemic')
              + theme_bw()
)
inc_reinf_fit <- (inc_reinf 
                  #+ xlim(as.Date('2021-07-01'), as.Date('2021-08-15'))
                  #+ xlim(1,50)
                  + ggtitle('Fitting period')
                  + scale_y_continuous(breaks = seq(0,2500,100), minor_breaks = seq(0,2500,100), limits = c(0, 2500))
)
inc_reinf_proj <- (inc_reinf 
                   #+ scale_x_date(name = 'Specimen receipt date', date_breaks = "months"
                   #               , date_labels = "%b", minor_breaks = NULL
                   #               , limits = c(as.Date('2021-08-15') + 1, NA)
                   #)
                   #+ xlim(50,100)
...
ggsave(inc_reinf_fit | inc_reinf_proj
       , filename = "simplot2.png", width = 6, height = 3)
